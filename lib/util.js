...
AWS.util = {
  ...
  appendParamsToURI: function appendParamsToURI( uri, params ) {
    var url = require('url');
    params = AWS.util.queryParamsToString( params );
    uri = url.parse(uri);
    uri.query = uri.query ? [uri.query, params].join('&') : params;
    uri.search = uri.query ? '?' + uri.query : null;
